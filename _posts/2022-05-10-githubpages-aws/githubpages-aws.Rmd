...
The following steps help when setting up a new website that is hosted on
**GitHub Pages**. Then, since GitHub Pages supports using custom
domains, there are the instructions to re-direct tje site's URL from the
...
-   on AWS: Go to **Route53**, under *'register domain'* then follow
    instructions to buy a domain (10$/12$ per year) `thepadrinoz.com`
...
-   (on Terminal/RStudio: Build the site locally (or see
    `./build_site.R`))
...
❗ Before adding/committing to repo `CNAME`, at every push, the custom
