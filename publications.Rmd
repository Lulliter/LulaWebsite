...
mybib_meta <- tibble::tribble( 
	  ~"url_pub", ~"url_pdf",~ "url_rg", ~"url_repo", ~"url_arch", ~"url_osf", ~"url_other" ) 
...
mybib_df2$url_pub[mybib_df2$category == "ARTICLE"] <- mybib_df2$url[mybib_df2$category == "ARTICLE"]

# blogs --> url_other
mybib_df2$url_other[mybib_df2$category == "ONLINE"] <- mybib_df2$url[mybib_df2$category == "ONLINE"]

# G20 proceedings -> url_pdf  
mybib_df2$url_pdf[mybib_df2$category == "PROCEEDINGS"] <- mybib_df2$url[mybib_df2$category =="PROCEEDINGS"]

# THESIS in --> repo 
mybib_df2$url_arch[mybib_df2$category == "THESIS"] <- mybib_df2$url[mybib_df2$category =="THESIS"]

# Work Paper in RG --> url_rg
mybib_df2$url_rg <- ifelse(str_detect(mybib_df2$url, "researchgate.net"),  mybib_df2$url, NA)

# Se c'e' 1 PFD in --> url_pdf may cause double-counting
mybib_df2$url_pdf <- ifelse(str_detect(mybib_df2$url, ".pdf"),  mybib_df2$url, NA)

# url_preprint ERA GIA SALVATO GIUSTO DA SOLO!!!!
...
mybib_df2$category2[mybib_df2$type == "Peer-reviewed" ] <- "peer-reviewed"
mybib_df2$category2[mybib_df2$type == "Working Paper" ] <- "working"
...
mybib_df2$category2[mybib_df2$type == "Invited Talk" ] <- "conference"
mybib_df2$category2[mybib_df2$type == "Presentation" ] <- "conference"
mybib_df2$category2[mybib_df2$type == "Thesis" ] <- "theses"


mybib_df2$category2[mybib_df2$type == "World Bank Blogs" ] <- "media"
...
mybib_df2 <- mybib_df2 %>% 
	select ( key,  file,
				...
				author, title, journaltitle,   doi,
				type, category, category2, 
				 url, url_pub, url_preprint, url_pdf, url_repo,   url_arch, url_rg, url_osf, url_other, 
				abstract,   image)


# names of column like Academicons 
...
names(mybib_df_acad)[names(mybib_df2) == "url_pdf"] <- "pdf"
names(mybib_df_acad)[names(mybib_df2) == "url_repo"] <- "github"
...
names(mybib_df_acad)[names(mybib_df2) == "url_arch"] <- "archive"
...
#' @param link_type Type of link ("elsevier", "pdf", "github", "archive", "researchgate", "osf", "open-access")
#' 
...
#  "elsevier"  "preprint"  "pdf" "github" "archive" "researchgate" "osf" "open-access"  
...
print_ref <- function(key_select, bib = mybib, bib_df = mybib_df_acad) {
	...
	elsevier <- link_button(key_select, "elsevier", "Journal", bib_df) # x
	preprint <- link_button(key_select, "preprint", "Preprint", bib_df) # ????
	`open-access` <- link_button(key_select, "open-access", "Other", bib_df) # x 
	github <- link_button(key_select, "github", "Repo", bib_df) # na 
	archive <- link_button(key_select, "archive", "Univ Archive", bib_df) # ????
	researchgate <- link_button(key_select, "researchgate", "Research Gate", bib_df)
	osf <- link_button(key_select, "osf", "OSF", bib_df)
	pdf <- link_button(key_select, "pdf", "PDF", bib_df)
	
	main_ref <- paste(ref, doi, sep = " ")
	buttons <- paste(elsevier, preprint, archive, `open-access`, github, researchgate, osf, pdf)
...
`r icon_link("google-scholar", "Google Scholar", "https://scholar.google.com/citations?hl=en&user=OBYla5gAAAAJ")`  
